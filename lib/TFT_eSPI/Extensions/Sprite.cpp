...
void TFT_eSprite::drawThickLine(int32_t x0, int32_t y0, int32_t x1, int32_t y1, uint32_t color, float wd) {
  ...
  for (wd = (wd + 1) / 2; ;) { /* pixel loop */
    drawPixel(x0, y0, fmax(0, 255 * (abs(err - dx + dy) / ed - wd + 1)));
    e2 = err;
    x2 = x0;
    if (2 * e2 >= -dx) { /* x step */
      for (e2 += dy, y2 = y0; e2 < ed * wd && (y1 != y2 || dx > dy); e2 += dx) drawPixel(x0, y2 += sy, fmax(0, 255 * (abs(e2) / ed - wd + 1)));
      if (x0 == x1) break;
      e2 = err;
      err -= dy;
      x0 += sx;
    ...
    if (2 * e2 <= dy) { /* y step */
      for (e2 = dx - e2; e2 < ed * wd && (x1 != x2 || dx < dy); e2 += dy) drawPixel(x2 += sx, y0, fmax(0, 255 * (abs(e2) / ed - wd + 1)));
      if (y0 == y1) break;
      err += dx;
      y0 += sy;
    }
